...
class GHLService {
  ...
  async getContacts(accessToken = null) {
    try {
      ...
      // Build client with override token if provided
  ...
  async getInstalledLocations(params = {}, accessToken = null) {
    try {
      const { companyId, appId, skip, limit, query, isInstalled, versionId, onTrial, planId } = params || {};
      if (!companyId || !appId) {
        throw new Error('companyId and appId are required');
      }
      const client = accessToken ? axios.create({
        baseURL: this.baseUrl,
        headers: {
      ...
      const response = await client.get('/oauth/installedLocations', {
        params: { companyId, appId, skip, limit, query, isInstalled, versionId, onTrial, planId }
      });
      return response.data;
    } catch (error) {
      console.error('Error fetching GHL installed locations:', error.response?.data || error.message);
      throw error;
    }
  }
