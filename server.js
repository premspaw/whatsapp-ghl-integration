    ...
    if (whatsappService && inboundRelay) {
        whatsappService.on('message', async (message) => {
            try {
                ...
                let inboundMime = null;
                let inboundCaption = null;
                if (message.hasMedia && typeof message.downloadMedia === 'function') {
                    try {
                        const m = await message.downloadMedia();
                        if (m && m.data && m.mimetype) {
                            inboundMime = m.mimetype;
                            inboundCaption = message.caption || '';
                            const ext = (m.mimetype.includes('jpeg') ? 'jpg' : (m.mimetype.includes('png') ? 'png' : (m.mimetype.includes('pdf') ? 'pdf' : (m.mimetype.includes('mp4') ? 'mp4' : 'bin'))));
                            ...
                            inboundMediaUrl = `${base}/uploads/${name}`;
                            inboundMediaType = m.mimetype.startsWith('image/') ? 'image' : (m.mimetype.startsWith('video/') ? 'video' : 'document');
                        }
                    } catch (_) { }
                }
                ...
                const savedMsg = await conversationManager.addMessage({
                    ...
                    mediaUrl: inboundMediaUrl || message.mediaUrl || null,
                    mediaType: inboundMediaType || message.type,
                    mimeType: inboundMime || null,
                    caption: inboundCaption || null,
                    direction: 'inbound'
                }, message.from);
...
app.use('/provider', require('./routes/providerRoutes')(whatsappService, conversationManager));
...
// Mount GHL OAuth routes at /api so that /oauth/callback becomes /api/oauth/callback
app.use('/api', require('./routes/ghlRoutes')(ghlService));
